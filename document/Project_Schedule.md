...
### 4.0 启动设置界面
- [x] `GameSettings.cs` - 设置管理器（分辨率/窗口模式持久化、16:9等比强制、F11全屏切换、首次启动自动检测屏幕分辨率）
- [x] `LaunchScreen.cs` - 启动设置界面脚本（分辨率下拉框/窗口模式切换/开始按钮）
- [x] `project.godot` - 配置 viewport 1920×1080 + stretch_mode=canvas_items + stretch_aspect=keep_aspect
- [ ] 启动场景搭建 `scenes/LaunchScreen.tscn`（需在 Godot 编辑器中按操作步骤搭建）
...
| 2026-04-11 | M2/M3 | 确立UI布局约束 | 需要统一分辨率和等比缩放规范 | 基准分辨率16:9（1920×1080）；窗口拉伸必须等比；使用锚点+容器布局 |
| 2026-04-11 | M4-4.0 | 规划启动设置界面 | Godot默认无启动配置对话框 | 新增4.0小节：分辨率选择+窗口模式切换+设置保存+启动场景 |
| 2026-04-11 | M4-4.0 | 完成GameSettings+LaunchScreen脚本开发 | 需要设置持久化+16:9等比强制 | GameSettings.cs（设置管理器，user://settings.json持久化，F11全屏切换，首次自动检测屏幕分辨率）；LaunchScreen.cs（启动界面脚本）；更新GameRoot/EventBus/Architecture |
