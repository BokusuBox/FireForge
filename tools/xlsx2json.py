...
def generate_table_manager_cs(all_tables=None):
    register_lines = []
    if all_tables:
        for table in all_tables:
            ...
            register_lines.append(f'        RegisterTable<{table_class}>("{table_name}", td => new {table_class}(td));')

    register_block = '\n'.join(register_lines) if register_lines else ''

    return f'''// 自动生成的代码 - 请勿手动修改
...
def main():
    ...
    manager_code = generate_table_manager_cs(all_tables)
